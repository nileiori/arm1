#ifndef __GOL_RTC_C__
#define __GOL_RTC_C__
...
uint8_t timeUpdate(rtc_parameter_struct *time)
{
    static uint8_t last_sec = 0xff;
    
    if(last_sec != time->second)
    {
        last_sec = time->second;
 
        RSOFT_RTC_SECOND    = time->second;
        RSOFT_RTC_MINUTE    = time->minute;
        RSOFT_RTC_HOUR      = time->hour;
        RSOFT_RTC_DAY       = time->date;
        RSOFT_RTC_MONTH     = time->month;
        RSOFT_RTC_YEAR      = time->year;
