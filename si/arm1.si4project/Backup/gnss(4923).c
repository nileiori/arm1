...
int gnss_BESTPOS_Buff_Parser(char * pData)
{
    ...
    switch(gCmdIndex)
    {
        ...
        case 9:	
        	break;
        case 10:				//字段1			解算状态
        	for(i=0;i<sizeof(sol_status)/sizeof(sol_status[0]);i++)
        	{
				if((NULL != strstr(pData, sol_status[i]))
				{
					break;
				}
        	}
        	hGPSBestPosData.calcState = (Resolve_TypeDef)atoi(sol_status[i]);
            break;
...
int gnss_AGRIC_Buff_Parser(char * pData)
{
    switch(gCmdIndex)
    {
        case 0:					//字段0
            ...
            {
				memcpy(hGPSAgricData.log_header.sync,pData,sizeof("#AGRICA"));
            }
        ...
        case 1:					//字段1
			...
			hGPSAgricData.log_header.CPUIDle = atoi(pData);
            break;
		case 2:					//字段2,3不解析
        case 3: 
            break;
        case 4:					//字段4 GPS周数
            hGPSAgricData.log_header.gpsWn = atol(pData);
            break; 
        case 5:					//字段5 GPS周内秒
            hGPSAgricData.log_header.gpsMs = atol(pData);
            break; 
        case 6:					//字段6,7不解析
        case 7: 
        case 8:					//字段8 GPS周闰秒
            hGPSAgricData.log_header.leapSec = atoi(pData);
            break; 
        case 9:					//字段9不解析
            break; 
