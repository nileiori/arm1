...
char* GNSS_Cmd_Kind_Parser(char* pData,uint16_t dataLen)
{
	char *pDst = NULL;
	uint8_t  frameEnd[2] = {0x0d, 0x0a};
    do
    {
        if(*pData == '#')
        {
...
int GNSS_Cmd_Parser(char* pData)
{
    ...
    //else if(NULL != strstr(pData, "$GNRMC"))
        ...
        gCmdTypeIndex = 5;
    else if(NULL != strstr(pData, "#BESTPOSA"))
        gCmdTypeIndex = 6;
...
//  GPSDataTypeDef tGPSData;
    ...
    for(;;)
    {
        if(gCmdIndex == 0)
        {
            //pDst = GNSS_Cmd_Kind_Parser(pRxBuf,dataLen);//命令起始符
            //if(NULL == pDst)
            //	return INS_ERROR;
            gCmdTypeIndex = GNSS_Cmd_Parser((char*)pData);//命令类型
        }
		if(gCmdTypeIndex == 0)
			return INS_ERROR;
        if(cmdKind != 0)
        {
        	//pDst ="#AGRICA,98,GPS,UNKNOWN,1,8338000,0,0,18,2;GNSS,";
            if(cmdKind == 1)
                pBuf = strtok((char*)pData, gnssSemicolonSeperator);
            else if(cmdKind == 2)
                pBuf = strtok((char*)pData, gnssCommaSeperator);
