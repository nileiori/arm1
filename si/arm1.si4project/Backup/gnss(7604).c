...
void GNSS_Buff_Parser(char* pData, uint16_t nCmdIndex)
{
	...
	{
		case 1:gnss_RMC_Buff_Parser(pData);break;
		case 2:gnss_GGA_Buff_Parser(pData);break;
		case 3:gnss_VTG_Buff_Parser(pData);break;
		case 4:gnss_ZDA_Buff_Parser(pData);break;
		case 5:gnss_Heading_Buff_Parser(pData);break;
		case 6:gnss_BESTPOS_Buff_Parser(pData);break;
		case 7:gnss_TRA_Buff_Parser(pData);break;
		default: break;
	}
}
...
    for(;;)
    {
        xQueueReceiveFromISR(GPSQueue,pRxBuf,NULL);         //待修改
        if(gCmdIndex == 0)
        {
...
            gCmdIndex ++;
        }
        memcpy(&hINSData.GNSSS_Data,&hGPSData,sizeof(GPSDataTypeDef));
        hINSData.GNSS_DataRead = 1;
        memset(pRxBuf,0,strlen(pRxBuf));
        vTaskDelay(5);
    }
}
