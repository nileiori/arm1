...
#define	Sensor_Scale 	65536.0
    ...
    while( 1 )
    {
        ...
        if((IMU_FRAME_HEADER_LSB == data_l) && (IMU_FRAME_HEADER_MSB == data_h))
        {
            ...
            if(sum == calSum)
            {
            	gd32_pin_write(GD32F450_PD2_PIN_NUM, PIN_HIGH);
            	gd32_pin_write(GD32F450_PD2_PIN_NUM, PIN_HIGH);
            	gd32_pin_write(GD32F450_PD2_PIN_NUM, PIN_LOW);
                ...
                mixData.accelGrp[1] = rs422_frame.data_stream.accelY * Accel_Scale / Sensor_Scale;
                ...
                mixData.accelGrp[2] = rs422_frame.data_stream.accelZ * Accel_Scale / Sensor_Scale;
                ...
                imuParseData.accelGrp[0] = rs422_frame.data_stream.gyroX * Rate_Scale / Sensor_Scale;
                ...
                mixData.gyroGrp[1] = rs422_frame.data_stream.gyroY * Rate_Scale / Sensor_Scale;
                imuParseData.accelGrp[1] = rs422_frame.data_stream.gyroY * Rate_Scale / Sensor_Scale;
                ...
                mixData.gyroGrp[2] = rs422_frame.data_stream.gyroZ * Rate_Scale / Sensor_Scale;
                imuParseData.accelGrp[2] = rs422_frame.data_stream.gyroZ * Rate_Scale / Sensor_Scale;
...
void frame_fill_gnss(uint8_t* pData, uint16_t dataLen)
{
    if(INS_EOK == gnss_parse(pData, dataLen))
    {
        ...
        if(0x1f == gnssSynFlg)
        {
        	gd32_pin_write(GD32F450_PC12_PIN_NUM, PIN_HIGH);
        	gd32_pin_write(GD32F450_PC12_PIN_NUM, PIN_HIGH);
        	gd32_pin_write(GD32F450_PC12_PIN_NUM, PIN_HIGH);
        	gd32_pin_write(GD32F450_PC12_PIN_NUM, PIN_HIGH);
        	gd32_pin_write(GD32F450_PC12_PIN_NUM, PIN_LOW);
	        gnssSynFlg = 0;
	        gnss_Fetch_Data();
	        xNavStatus = 2;
