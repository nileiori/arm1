...
{
	CombineDataTypeDef *pGnss = (CombineDataTypeDef*)gnss;

	p->gpsweek = pGnss->gnssInfo.gpsweek;
	p->gpssecond = pGnss->gnssInfo.gpssecond;
	p->heading = pGnss->gnssInfo.Heading;
	p->pitch = pGnss->gnssInfo.Pitch;
	p->roll = pGnss->gnssInfo.Roll;
	p->latitude = pGnss->gnssInfo.Lat;
	p->longitude = pGnss->gnssInfo.Lon;
	p->altitude = pGnss->gnssInfo.Altitude;
	p->ve = pGnss->gnssInfo.ve;
	p->vn = pGnss->gnssInfo.vn;
	p->vu = pGnss->gnssInfo.vu;
	p->baseline = pGnss->gnssInfo.baseline;
	p->nsv = pGnss->gnssInfo.StarNum;
	p->gnssstatus = pGnss->gnssInfo.rtkStatus;
	p->utc = pGnss->gnssInfo.timestamp;//?
	p->supportposvelstd = 1;
	p->latstd = pGnss->gnssInfo.LatStd;
	p->logstd = pGnss->gnssInfo.LonStd;
	p->hstd = pGnss->gnssInfo.AltitudeStd;
	p->vestd = pGnss->gnssInfo.vestd;
	p->vnstd = pGnss->gnssInfo.vnstd;
	p->vustd = pGnss->gnssInfo.vustd;
