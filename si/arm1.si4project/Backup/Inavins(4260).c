...
{
	int i,j;
	double traMat[9] = { 0,0,-1,1,0,0,0,1,0 };// 双矢量定姿的转换矩阵
	double gnMat[9];// 以gn为主参考矢量，构建的右矩阵
...
void CoarseAlign(double* sumWm, double* sumVm, double* pos, int count, double* qnb0)
{
	...
	for (i = 0; i < 3; i++)gn[i] = wnie[i]= 0.0;//初始化为0
...
void UpdateEarth(EARTH* eth, double* vn, double* pos)  //更新大地数据
{
	double sq, sq2;
	double temp[3];
...
unsigned int StartCoarseAlign(I_NAV_INS * navins,  I_NAV_GNSS_RESULT *gnss)
{
	...
	GnssPosition[2] = 	gnss->altitude ;//高程单位m
...
void InsInit(INSRESULT *ins, double ts, double* qnb, I_NAV_GNSS_RESULT *pNAV_GNSS_RESULT,double *GnssArmLength,I_NAV_INS * navins) //惯导初始化
{
	...
	ins->pos[2]	=	pNAV_GNSS_RESULT->altitude ;//高程单位m
	for (i = 0; i < 4; i++) {
		ins->qnb[i] = qnb[i];
		if (i < 3) {
...
void KfInit(KALMAN* kf)  //卡尔曼滤波初始化
{
	...
	for (i = 0; i < NA; i++)
	{
		...
		if (i >= 0 && i < 2)
			...
			//InitPQ(kf, 0.0, 1 * 1, 0.0, i);//注意此次用了1 * 1,由于高程使用米单位，不需要除以Re
	...
	for (i = 0; i < NB; i++)
	{
		if (i < 3)
			...
			//kf->RK[i] = 0.1 * 0.1;
			kf->RK[i] = 0.3 * 0.3;//? 高程误差, RTK，PPP理论高程误差大约是3倍的水平误差
		else
			kf->RK[i]=HEAD_VAR;
	}
	InitHk(kf);
...
void UpdateKfRHuge(KALMAN* kf)
{
	...
	mposvar[2]	=	5.0*5.0*1.0e8;//rtk 高程误差0.1m
...
void UpdateKfR(KALMAN* kf ,I_NAV_GNSS_RESULT *pNavGnss)
{
	...
	else if(0 == pNavGnss->supportposvelstd && NAV_GNSS_STATUS_RTK_FIX == pNavGnss->gnssstatus)
	{
		...
		mposvar[2]	=	0.1*0.1;//rtk 高程误差0.1m
	}
	else 
	{
		for(i=0;i<3;i++)
...
//------------------------------------------------------------------------------------
	...
	Cnscl(imu, phim_, dvbm_);//双子样特殊处理
	matrixSum(phim_, ins->eb, 3, 1, -nts, phim);// kg*phim-eb*nts   陀螺角速度
	matrixSum(dvbm_, ins->db, 3, 1, -nts, dvbm);// ka*dvbm-db*nts 加速度
//------------------------------------------------------------------------------------
	matrixSum(ins->vn, ins->an, 3, 1, nts / 2, vn01);//计算nts这段时间平均速度
	matmul("NN", 3, 1, 3, 1.0, ins->Mpv, vn01, 0.0, dpos);
	matrixSum(ins->pos, dpos, 3, 1, nts / 2, pos01);//计算nts这段时间位置

	UpdateEarth(&navins->earth, vn01, pos01);//更新大地数据
...
//---------------pos update---------------------------------------------------------------
	...
	dpos_[0] = ((ins->vn[1] + vn1[1]) / navins->earth.RMh) * (nts / 2);//使用平均速度计算位置变化
...
unsigned char KfFk(INSRESULT * ins, double* Fk,I_NAV_INS * navins)//卡尔曼滤波、构建FK矩阵,计算一次状态更新矩阵
{
	int i,j;
...
unsigned char  KfTimeUpdate(KALMAN* kf, double* Fk, double nts,int m) //kf时间更新
{
...
#if 1
	if (MOTION_STATUS_HEADING==g_STATIC_DETECTION.state)//当前没有改模式
	{
		for (i = 0, k = 0; i < 6; i++)
		{
...
void MatrixRk(double* R,int m, double* Rk)
{
	...
	//PrintOutMatrixMsg(m,m,Rk, "R");//打印R矩阵
}
...
unsigned char GnssInsFusion(INSRESULT* ins, I_NAV_GNSS_RESULT* gnss,KALMAN* kf ,double *lever ,double dt) //GnssIns融合、杆臂、时间同步补偿、量测更新
{
	int n, m;
	double *Zk, *Hk, *Rk;
	double GnssPosition[3];
...
void KfFeedback(KALMAN* kf, INSRESULT* ins)  //卡尔曼滤波反馈
{
	double phi[NN];
	int i;
...
double averageBias(double cnt,double meas,double average)//平均偏移
{
...
double compute_var(double* sample,int size)  //计数变量
{
	int i;
	double ave;
...
void InitialNavIncParm()
{
	...
	g_NAV_INS.insStatus 			=	NAV_INS_STATUS_WAIT;//暂时从NAV_INS_STATUS_WAIT开始
}
...
void InterfaceKalman()
{
	...
	{
		case NAV_INS_STATUS_IDLE:   //0空闲
		{
		      
		}
		break;

		case NAV_INS_STATUS_START:  //1开始导航
		{
			g_NAV_INS.insStatus=	NAV_INS_STATUS_WAIT;
		}
		break;

		case NAV_INS_STATUS_WAIT:  //2等待稳定状态
		{
			if(1 == g_NAV_GNSS_RESULT.gnssstartflag)//gps已经启动可以进行融合惯导
			{
		...
		case NAV_INS_STATUS_ROUTH_ALIGN:  //2等待稳定状态
		{
			//粗对准
		...
		case NAV_INS_STATUS_KEEP:  //进入导航状态
		{
			//惯导数据更新
			...
			if(INS_FUN_RETURN_SUCESS == KfFk(&g_NAV_INS.ins,g_NAV_INS.Fk,&g_NAV_INS))
			{
				...
				if(MOTION_STATUS_MOVING != g_STATIC_DETECTION.state)//当前非动态
				{
					//GNSS RTK固定解
				...
				if(INS_FUN_RETURN_SUCESS ==KfTimeUpdate(&g_NAV_INS.kf, g_NAV_INS.Fk,TS, 1))
				{
					...
					{
						//dengwei 单点定位、rtk固定解的量测误差不同，需要更新R 矩阵,
						UpdateKfR(&g_NAV_INS.kf,&g_NAV_GNSS_RESULT);
					}
					else//失锁情况下设置R矩阵误差无限大
					{
