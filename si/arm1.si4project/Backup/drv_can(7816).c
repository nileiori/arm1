...
void Can_FormatData(can_receive_message_struct rxMsg, TCanData* data)
{
...
void gd32_bxcan_init(void)
{
    ...
    // Filter
    bxcan_format_filter(&config.filter[0], 0, FrameType_Data, IDType_Extend, 0x12);
    bxcan_format_filter(&config.filter[1], 1, FrameType_Data, IDType_Standard, 0x18);
    bxcan_filter_config(config);
    //Initialize CAN2
    config.ch = CAN_2;
    ...
    bxcan_format_filter(&config.filter[0], 0, FrameType_Data, IDType_Extend, 0x12);
    bxcan_format_filter(&config.filter[1], 1, FrameType_Data, IDType_Standard, 0x18);
    bxcan_format_filter(&config.filter[2], 2, FrameType_Data, IDType_Extend, 0x1ABB);
    bxcan_filter_config(config);
}
