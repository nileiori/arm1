...
EventGroupHandle_t xGnssEventBits;
...
#define GNSS_SYNC_BITS 		( BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5))
...
//        uxReturn = xEventGroupSync( xGnssEventBits,
//		BIT(0), 
//		GNSS_SYNC_BITS, 
//		xTicksToWait );
//		if( ( uxReturn & GNSS_SYNC_BITS ) == GNSS_SYNC_BITS )
//		{
//	        gnss_Fetch_Data();
//	        xNavStatus = 2;
//    		xQueueSend(xNavQueue, &xNavStatus, 0);
...
int GNSS_Cmd_Parser(char* pData)
{
    ...
    else if(0 == strncmp("$GNRMC", (char const *)pData, 6))
    {
	    gnssSynFlg |= (0x1 << 0);
	    gCmdTypeIndex = 1;
//	    xEventGroupSync( xGnssEventBits, BIT(1), GNSS_SYNC_BITS, portMAX_DELAY );
    }
    ...
    else if(0 == strncmp("$GNGGA", (char const *)pData, 6))
    {
	    gnssSynFlg |= (0x1 << 1);
	    gCmdTypeIndex = 2;
//	    xEventGroupSync( xGnssEventBits, BIT(2), GNSS_SYNC_BITS, portMAX_DELAY );
    }
    ...
    else if(0 == strncmp("$GNZDA", (char const *)pData, 6))
    {
	    gnssSynFlg |= (0x1 << 2);
	    gCmdTypeIndex = 4;
//	    xEventGroupSync( xGnssEventBits, BIT(3), GNSS_SYNC_BITS, portMAX_DELAY );
    }
    else if(0 == strncmp("#HEADINGA", (char const *)pData, 9))
    {
	    gnssSynFlg |= (0x1 << 3);
	    gCmdTypeIndex = 5;
//	    xEventGroupSync( xGnssEventBits, BIT(4), GNSS_SYNC_BITS, portMAX_DELAY );
    }
    else if(0 == strncmp("#BESTPOSA", (char const *)pData, 9))
    {
	    gnssSynFlg |= (0x1 << 4);
	    gCmdTypeIndex = 6;
//	    xEventGroupSync( xGnssEventBits, BIT(5), GNSS_SYNC_BITS, portMAX_DELAY );
    }
