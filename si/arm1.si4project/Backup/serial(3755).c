...
static uint32_t ins_serial_open(struct serial_device *serial, uint16_t oflag)
{
    ...
    if (serial->serial_rx == NULL)
    {
        if (oflag & DEVICE_FLAG_INT_RX)
        {
            ...
            rx_fifo = (serial_rx_fifo*) pvPortMalloc (sizeof(serial_rx_fifo) +
                                                   serial->config.bufsz);
            configASSERT(rx_fifo != NULL);
    ...
    if (serial->serial_tx == NULL)
    {
        if (oflag & DEVICE_FLAG_INT_TX)
        {
            ...
            tx_fifo = (serial_tx_fifo*) pvPortMalloc (sizeof(serial_tx_fifo) +
                                                   serial->config.bufsz);
            configASSERT(tx_fifo != NULL);
...
uint32_t hw_serial_register(struct serial_device *serial)
{
    ...
    ins_serial_open(serial,DEVICE_FLAG_INT_RX/* | DEVICE_FLAG_INT_TX*/);
