...
typedef enum 
{
    ...
    MOTION_STATUS_HEADING   =2, //航向约束模式,暂时未使用
}EMotionStatus;

//函数返回
typedef enum 
{
    INS_FUN_RETURN_FAIL = 0, //ins算法返回失败  
    INS_FUN_RETURN_SUCESS =1 //算法返回成功
}EInsFunReturnVal;
...
typedef struct INSRESULT_T{
	...
	double vn[3];//当前速度
	double pos[3];//当前位置
	double vnstd[3];//当前速度精度
	double posstd[3];//当前位置精度
	double eb[3], db[3];					// 陀螺的漂移和加速度计的偏差
	double an[3], fb[3], fn[3];//导航系加速度、载体坐标系力、导航系力
	double web[3], wnb[3];//web 载体坐标系相对地球坐标系的角速度；wnb载体坐标系在导航坐标系角速度
	double Mpv[3* 3];
	double learm[3];
}INSRESULT;
...
extern "C" {
#endif
void InterfaceKalman(void);
void InitialNavIncParm(void);
unsigned int GetNavIncData(IMUDATA *p, void *comb);
unsigned int GetNavGnssData(I_NAV_GNSS_RESULT *p, void *gnss);

IMUDATA * GetNavIncImuPointer(void);
I_NAV_INS * GetNavIncPointer(void);
STATIC_DETECTION * GetStaticDetectPointer(void);
COMPENSATE_PARAMS * GetCompensateParmsPointer(void);
...
unsigned char GnssInsFusion(INSRESULT* ins, I_NAV_GNSS_RESULT* gnss,KALMAN* kf ,double *lever ,double dt) ;
void KfFeedback(KALMAN* kf, INSRESULT* ins) ;
int StopNavigation(void);
//打印信息
void PrintOutGNSSMsg(void);
void PrintOutInsMsg(void);
