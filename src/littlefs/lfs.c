...
void lfs_selftest(void) {
    ...
    if (err) {
        lfs_format(&m_lfs, &cfg);
        lfs_mount(&m_lfs, &cfg);
    }

    // read current count
    uint32_t boot_count = 0;
    lfs_file_open(&m_lfs, &file, "boot_count", LFS_O_RDWR | LFS_O_CREAT);
    lfs_file_read(&m_lfs, &file, &boot_count, sizeof(boot_count));

    // update boot count
    boot_count += 1;
    lfs_file_rewind(&m_lfs, &file);
    lfs_file_write(&m_lfs, &file, &boot_count, sizeof(boot_count));
