...
void ch378_port_init( void )
{
    ...
    gd32_pin_mode(CH378_RST1_PIN_NUM, PIN_MODE_OUTPUT_PULLUP);
    gd32_pin_write(CH378_RST1_PIN_NUM, PIN_HIGH);//rst1脚拉低
    hw_spi_init(SPI3);
    //绑定中断
    gd32_pin_mode(CH378_INT_PIN_NUM, PIN_MODE_INPUT_PULLUP);
    //pin_irq_install(  CH378_INT_PIN_NUM, PIN_MODE_INPUT,
...
uint8_t ch378_wake_up(uint8_t sleep_mode)
{
    ...
    if(FULL_SLEEP_MODE == sleep_mode)
    {
        ...
        disk_status = e_DISK_WAKE_UP;
    }

    return ret;
}

void ch378_disk_connect(void)
{
    uint8_t cmd;
...
void ch378_disk_mount(void)
{
    uint8_t cmd;

    cmd = CMD0H_DISK_MOUNT;//检查磁盘是否就绪
    spi_send(&cmd, 1, 1, 1);
    disk_status = e_DISK_MOUNT;
}
...
void ch378_dir_create(const char* name)
{
    uint8_t cmd;

    if(ERR_SUCCESS != ch378_set_filename(name))return;//文件名非法

    cmd = CMD0H_DIR_CREATE;//新建目录
    spi_send(&cmd, 1, 1, 1);
    disk_status = e_DISK_DIR_CREATE;
}

void ch378_file_create(const char* name)
{
    uint8_t cmd;
    ...
    disk_status = e_DISK_FILE_CREATE;
}

void ch378_file_open(const char* name)
{
    uint8_t cmd;
...
void ch378_task(void* arg)
{
    ...
    sendBuf[0] = CMD11_CHECK_EXIST;
    sendBuf[1] = 0xa5;
    sendBuf[2] = 0xff;
    dataRecv = spi_send(sendBuf, 3, 1, 1);
    if(dataRecv != 0x5a)//取反
    {
        //do something
    }
	...
	do
	{
		if(0 == gd32_pin_read(CH378_INT_PIN_NUM))
			break;
	}
	while (1);
	ch378Status = ch378_get_intStatus();if( ch378Status == CMD_RET_SUCCESS ) 
    //vTaskDelay(50);
    //spi_recv(&dataRecv, 1, 1, 1);
//    sendBuf[0] = CMD11_SET_USB_MODE;
//    sendBuf[1] = 0x06;
//    sendBuf[2] = 0xff;
//    dataRecv = spi_send(&cmd, 3, 1, 1);
...
//
    ...
    for( ;; )
    {
        ...
        {
            //获取CH378的中断状态并通知CH378取消中断请求
            ch378Status = ch378_get_intStatus();
            ...
            switch( dataRecv )
            {
                ...
                case e_DISK_DIR_CREATE:
                    if(ERR_SUCCESS == status)
                    {

                    }
                    else if(ERR_FOUND_NAME == status)
                    {
