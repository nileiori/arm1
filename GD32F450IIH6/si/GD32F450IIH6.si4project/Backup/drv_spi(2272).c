...
static struct ins_spi_configuration spi_cfg =
{
    ...
    .data_width = 8,
    .max_hz  = 10000000ul	/* ch378 max freq 30M(spi) */
};
...
void hw_spi_init(uint32_t spi_periph)
{
    ...
    gpio_af_set(GPIOG, GPIO_AF_6, GPIO_PIN_11 | GPIO_PIN_13);
    gpio_mode_set(GPIOG, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_PIN_11 |  GPIO_PIN_13);
    gpio_output_options_set(GPIOG, GPIO_OTYPE_PP, GPIO_OSPEED_MAX, GPIO_PIN_11 |  GPIO_PIN_13);
    ...
    gpio_mode_set(GPIOG, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_PIN_14);
