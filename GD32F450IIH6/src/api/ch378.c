...
void ch378_port_init( void )
{
    ...
    gd32_pin_write(CH378_RST1_PIN_NUM, PIN_HIGH);//rst1脚拉高
    hw_spi_init(SPI3);
    //绑定中断
    //gd32_pin_mode(CH378_INT_PIN_NUM, PIN_MODE_INPUT_PULLUP);
...
//    if( ch378Status == CMD_RET_SUCCESS )
        ...
        for( ;; )
        {
            if(pdTRUE == xQueueReceive( xCh378Queue, &dataRecv, pdMS_TO_TICKS(100)))
            {
                ...
                switch( dataRecv )
                {
                    case e_DISK_MODE_SET:
                        if(CMD_RET_SUCCESS == status)
                        {
                            ch378_disk_connect();
                        }
