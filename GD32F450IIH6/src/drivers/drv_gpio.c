...
void gd32_pin_mode(uint32_t pin, uint32_t mode)
{
    ...
    switch(mode)
    {
		...
		case PIN_MODE_OUTPUT_PULLUP:
            /* output setting */
            gpio_mode_set(index->gpio_periph, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, index->pin);
            gpio_output_options_set(index->gpio_periph, GPIO_OTYPE_PP, GPIO_OSPEED_MAX, index->pin);
            break;
        case PIN_MODE_OUTPUT_PULLDOWN:
            /* output setting */
            gpio_mode_set(index->gpio_periph, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLDOWN, index->pin);
            gpio_output_options_set(index->gpio_periph, GPIO_OTYPE_PP, GPIO_OSPEED_MAX, index->pin);
            break;
...
//extern uint8_t ch378_get_intStatus( void );
void EXTI4_IRQHandler(void)
{
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
	
    GD32_GPIO_EXTI_IRQHandler(4);
    //disk_status = ch378_get_intStatus();
